...
### Added
- **Phase 9 (tests for roles, bans, moderator news, blocked-user):** New fixture `banned_user`. API: `test_login_banned_user_returns_403`, `test_me_banned_user_returns_403`; web: `test_login_banned_user_redirects_to_blocked`, `test_blocked_page_returns_200`. Users API: `test_users_get_self_banned_returns_403`, assign role (admin 200, non-admin 403, invalid 400), ban (admin 200, non-admin 403), unban (admin 200, non-admin 403), list/get admin includes ban fields. News: `test_news_post_with_admin_returns_201`. Existing tests for moderator news write and user 403 unchanged.
- **Phase 8 (Postman):** Collection and README updated: roles user/moderator/admin; news write requires moderator or admin; editor references replaced with moderator. New requests: Users Assign role (PATCH …/role), Users Ban (POST …/ban), Users Unban (POST …/unban). Descriptions and test scripts updated; Login 403 for banned documented.
- **Phase 7 (backend user data for admin views):** User list (admin) and user get/update (admin) responses now include ban fields (`is_banned`, `banned_at`, `ban_reason`) via `to_dict(include_ban=True)`. Assign-role, ban, and unban responses already did. Future frontend admin screens can show role and ban state without further backend changes.
- **Phase 6 (banned-user enforcement):** API login rejects banned users with 403 and `{"error": "Account is restricted."}` and logs `login_blocked_banned`. `/auth/me` and `GET /api/v1/users/<self>` return 403 for banned users with valid JWT. Web login and session protection already redirect banned users to `/blocked` (Phase 3). BackendApi.md: Login and Me document 403 for banned/restricted accounts.
- **Phase 5 (admin user-management API):** Admin-only endpoints: `PATCH /api/v1/users/<id>/role` (body: `role` — user, moderator, or admin), `POST /api/v1/users/<id>/ban` (optional body: `reason`), `POST /api/v1/users/<id>/unban`. Service layer: `assign_role`, `ban_user`, `unban_user` in user_service; role validated against user/moderator/admin; self-ban prevented. Responses include user with `include_ban=True`. Activity logs: user_role_changed, user_banned, user_unbanned. BackendApi.md updated (sections 4.5–4.7; editor→moderator/admin in roles).
- **Phase 4 (news write access):** News create, update, publish, unpublish, and delete are enforced as moderator-or-admin only via `@require_jwt_moderator_or_admin` and centralized `current_user_can_write_news()` (banned users excluded). Public read unchanged; ordinary users receive 403 on write endpoints. Wording and docstrings use moderator/admin (no editor references in active code).
